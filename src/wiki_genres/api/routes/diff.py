...
async def diff(
    since: datetime = Query(..., description="ISO 8601 timestamp. Return genres changed after this moment."),
) -> DiffResult:
    ...
    async with session_scope() as session:
        rows = (await session.execute(
            text("""
                SELECT id, wikipedia_title, wikidata_qid, last_changed_at,
                       first_seen_at
                FROM wg_genres
                WHERE last_changed_at > :since
    ...
    entries = [
        DiffGenreEntry(
            ...
            change_type=(
                "added" if r["first_seen_at"] and r["first_seen_at"] > since
                else "updated"
            ),
